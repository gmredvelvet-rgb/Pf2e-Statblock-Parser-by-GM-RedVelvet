/**
 * PF2e Statblock Parser — licence prompts and the free-trial reminder.
 *
...
export default class LicenseUI {
  ...
  static show({ autoHide = false } = {}) {
    ...
    card.id = CARD_ID;
    card.className = `vne-sbp-license-card ${licensed ? "is-active" : "is-trial"}`;
  ...
  static #header(badge = "") {
    return `
      <header>
        ...
        ${badge ? `<span class="vne-sbp-license-badge">${badge}</span>` : ""}
      </header>`;
  }
  ...
  static #trialMarkupGM() {
    return `
      ...
      <p class="vne-sbp-license-status">${L("TrialIntro")}</p>
      <p class="vne-sbp-license-note">${L("TrialPitch")}</p>
      <button type="button" data-action="connect" class="vne-sbp-license-primary">
        <i class="fa-brands fa-patreon" aria-hidden="true"></i> ${L("Connect")}
      </button>
      <button type="button" data-action="code">${L("HaveCode")}</button>
      <button type="button" data-action="dismiss" class="vne-sbp-license-dismiss">${L("Later")}</button>`;
  }
  ...
  static #trialMarkupPlayer() {
    return `
      ${LicenseUI.#header(L("TrialBadge"))}
      <p class="vne-sbp-license-status">${L("TrialIntroPlayer")}</p>
      <p class="vne-sbp-license-note">${L("TrialPlayerNote")}</p>
      <button type="button" data-action="dismiss" class="vne-sbp-license-dismiss">${L("Close")}</button>`;
  }
  ...
  static #activeMarkup() {
    ...
    return `
      ${LicenseUI.#header()}
      <p class="vne-sbp-license-status">${status}</p>
      <p class="vne-sbp-license-note">${L("ReleaseHint")}</p>
